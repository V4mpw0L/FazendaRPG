...
export default class FarmSystem {
    ...
    plant(index, cropId) {
        ...
        if (!this.skillSystem.meetsLevelRequirement('farming', cropData.requiredLevel || 1)) {
            return {
                success: false,
        ...
        // Dispatch event
        window.dispatchEvent(new CustomEvent('farm:planted', {
            detail: { index, cropId }
        }));
    ...
    harvest(index) {
        ...
        // Dispatch event
        window.dispatchEvent(new CustomEvent('farm:harvested', {
            detail: {
                index,
                cropId: cropData.id,
                amount,
    ...
    fertilize(index) {
        ...
        // Dispatch event
        window.dispatchEvent(new CustomEvent('farm:fertilized', {
            detail: { index }
        }));
    ...
    clearPlot(index) {
        ...
        // Dispatch event
        window.dispatchEvent(new CustomEvent('farm:cleared', {
            detail: { index }
        }));
    ...
    checkCrops() {
        ...
        for (let i = 0; i < this.plotCount; i++) {
            ...
            if (plot && plot.crop && !plot.wasReady && this.isPlotReady(i)) {
                ...
                window.dispatchEvent(new CustomEvent('farm:cropReady', {
        ...
        if (readyCount > 0) {
            ...
            window.dispatchEvent(new CustomEvent('farm:cropsReady', {
