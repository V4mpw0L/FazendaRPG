...
export default class QuestSystem {
    ...
    isQuestAvailable(questId) {
        ...
        if (questData.requiredLevel && this.player.data.level < questData.requiredLevel) {
            return false;
        }
    ...
    getQuestLastCompleted(questId) {
        const completed = this.player.data.quests.completed.find(q =>
            typeof q === 'object' && q.id === questId
        );
        ...
        progress.objectives.forEach(objective => {
            if (objective.type === objectiveType) {
                switch (objectiveType) {
                    ...
                    case 'woodcut':
                    case 'forage':
                        if (!objective.cropId || objective.cropId === data.itemId || !data.itemId) {
    ...
    completeQuest(questId) {
        ...
        if (questData.repeatable) {
            ...
            this.player.data.quests.completed = this.player.data.quests.completed.filter(
