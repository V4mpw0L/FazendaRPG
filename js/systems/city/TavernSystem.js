...
export default class TavernSystem {
  ...
  buyMeal(mealType) {
    ...
    console.log(
      `🍺 Ate ${mealType} meal: +${energyRestored} energy, +${healthRestored} health`,
    );

    // Dispatch event
    window.dispatchEvent(
      new CustomEvent("tavern:meal", {
        detail: {
          mealType,
          cost: meal.cost,
          energyRestored,
  ...
  canRest() {
    return (
      this.player.data.gold >= this.getRestPrice() &&
