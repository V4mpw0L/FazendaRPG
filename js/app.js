...
window.FazendaRPG.debug = {
    ...
    addItem: (itemId, amount) => window.FazendaRPG.engine?.inventorySystem.addItem(itemId, amount),
    setEnergy: (amount) => {
    ...
    levelUpSkill: (skill, levels) => {
        ...
        if (engine && engine.skillSystem) {
            const xpNeeded = engine.skillSystem.xpTable[
